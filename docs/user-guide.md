...
鍵盤輸入使用 ASCII：POJ 的點右音 `o͘` 請打 `oo`，上標鼻音 `ⁿ` 請打 `nn`；候選註解和全羅 POJ 輸出會顯示正式字形。
...
- 拍台文本來就允許完全省略第 1／4 調，省略仍是預設。`x` 是選配的顯式鍵：開音節標 1、促聲尾標 4，與 `v`＝2／8 同一機制；要精確定位或收斂 `-h` 尾的 4／8 時才用。
...
4. **第 1／4 調預設不打。** `tai`、`taid` 並存，不必為陰平多按一鍵；要精確時 `x`＝1／4（選配）。
...
| 第 8 調 | 7.24% | `v` | 入聲尾 `-p/-t/-k/-h` 即可判定；顯式第 4 調用 `x` |
| 第 9 調 | 0.02% | `q` | 極少用，放最遠 |
| 第 1／4 調 | （無標） | `x`（選配） | 省略仍是預設；顯式時開音節＝1、促聲尾＝4 |

`v` 同時承擔第 2＋第 8 調約 25.88%，`x` 對稱承擔第 1＋第 4 調（選配）。`z`→`ts`、`zh`→`tsh` 是高頻聲母少一鍵；組字列仍顯示你打的 `z`／`zh`，由輸入法對到詞典的 `ts`／`tsh`。`f` **只在 Telex 方案當音節連字符**（`taidfgiv` → `tai5-gi2`），主方案的 `ph`/`f` 模糊不受影響。
...
入聲看韻尾是不是 `-p/-t/-k/-h`：是則 `v`＝8、`x`＝4，否則 `v`＝2、`x`＝1。
...
| `w` | 第 7 聲 |
| `q` | 第 9 聲 |
| `x` | 第 1 聲；音節以 `-p/-t/-k/-h` 收尾時為第 4 聲（選配；省略仍是預設） |
| `z` / `zh` | `ts` / `tsh`（高頻聲母少一鍵） |
| `f` | 音節連字符（等同 `-`） |
...
1. **入聲 `-h` 尾的 4／8 收斂**：免調打 `tsioh` 時「借 (tsioh4)」和「石 (tsioh8)」共用同一個無調拼式；打 `tsiohx` 直接指定第 4 調（借）、`tsiohv` 指定第 8 調（石），不必翻頁找字。
2. **強制音節邊界**：連打 `axi` 會剖析成 `a1 + i` 兩個音節，如果你想打的就是這兩個音節、不想讓引擎臆成別的切法，用 `x` 把第一音節定調。
3. **第 1 調精確查找**：`ix` → `i1`（伊），等同打數字 `i1`，手不必離開字母列。

平常仍然建議省略第 1／4 調——`x` 是「要精確時才按」的選配鍵，與 `v`（2／8）對稱。
...
全羅模式特有功能：
- **半形標點**：標點符號自動輸出半形（`.` `,` `!` `?` `;` `:` `"` `'` `(` `)` `[` `]` `{` `}` `<` `>` 等），符合羅馬字書寫慣例；`Ctrl+標點` 可臨時輸出全形（如 `Ctrl+.` → `。`）。漢羅模式反之：標點預設全形，`Ctrl+標點` 臨時輸出半形（如 `Ctrl+,` → `,`）
...
推薦不只是標記：**有標記的候選會往前移動固定格數（有界位移）**——

- LKK 推薦（◆ 漢字／★ 羅馬字）最多前進 3 格；
- 教育部700字推薦（◆）最多前進 1 格；
- 字典詞頻仍是主要順序：任何徽章都蓋不過明顯更常見的詞；
- **整句組字候選不受影響**：連打整句時，引擎組出的整句永遠在帶徽章的
  單詞之上（sentence 錨定），徽章只在整句之後的候選之間重排。
...
拍台文內建 **90,487 筆華語→台語對照表**（從 ChhoeTaigi 資料庫與教育部學科術語、臺灣地名資料建置），即使華語字和台語字不同也能正確轉換：
...
主字典同時納入兩個教育部官方來源：「學科術語臺灣台語對譯」（5,500+ 詞，8 大領域）佮「以本土語言標注臺灣地名」（9,000+ 詞：鐵路／捷運／高鐵／台灣好行站名＋行政區、聚落、自然實體、公共設施、街道）。TL／POJ 拼法攏會通，聲調免拍：

| 你想打的 | TL 免調 | POJ 免調 | 實測候選位置 |
|----------|---------|----------|--------------|
| 員林 | `uan-lim` | `oan-lim` | 前兩名 |
| 光合作用 | `kong-hap-tsok-iong` | `kong-hap-chok-iong` | 前兩名（◆ 推薦） |
...
| `Ctrl+標點` | 標點寬度臨時翻轉 | 漢羅模式出半形（`Ctrl+,` → `,`）；全羅模式出全形（`Ctrl+.` → `。`） |
...
分類內用 `Space`／`asdf` 選字，`Esc` 取消。原來的台羅調號（á à a̍）與台文標點清單仍在目錄後面，翻頁即可。
...
| `w` | 7 | 第 7 調 |
| `q` | 9 | 第 9 調 |
| `x` | 1／4 | 舒聲第 1 調；`-p/-t/-k/-h` 尾第 4 調（選配；省略仍是預設） |
| `z` | `ts` | 聲母簡寫；組字仍顯示 `z` |
...
記法二：`q` 像 9；`y` 三叉＝3；`v` 一分為二＝2／入聲 8；`d`＝第／D＝5；剩下 `w`＝7（不要用羅馬數字 V＝5）；`x` 打叉定案＝1／入聲 4。
...
若按 `` ` `` 符號選單正常、但 `` `e `` 沒有反應，通常是目前機器上的安裝是較舊版本（Emoji 分類瀏覽自 0.9.1 起提供），或更新檔案後尚未重新部署：
