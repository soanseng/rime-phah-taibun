...
Telex 原是電傳打字（teleprinter / telex）時代的編碼想法：線路只能送 ASCII 字母，但越南文國語字有聲調（sắc、huyền、hỏi、ngã、nặng）和母音附加符號（â、ê、ô、ă、ơ、ư）。解決辦法是**在字母後面多打一顆字母當調號或變音**，例如：
...
字典沒收的詞組（人名、地名、新詞，如 kiô-tiáⁿ）整串直接打，候選會逐音節組合：
...
- **不會**自動把你打過的內容存成新詞——使用者字典只記錄你選過的字詞；查無的詞組改以「逐音節組字」現場組合（見[基本輸入](#二基本輸入)）。
- 學習資料存在 Rime 的個人使用者字典裡，跟著你的設定檔走。

### 忘記選錯的字

如果某個候選因為手誤被學起來、一直排在前面，想把它降回去：在候選清單中**反白該候選**後按 `Shift+Delete`（多數前端如 fcitx5-rime、ibus-rime 支援；實際按鍵以你的前端設定為準）。
...
| `F4` / `` Ctrl+` `` | Rime 方案選單 | 切換輸出模式（漢羅/全羅、TL/POJ） |
...
0.4.0 正式安裝包沒有綁定 rime-ice，也沒有啟用 Emoji OpenCC 或 `melt_eng` 英文字典；只把 rime-ice 檔案複製到使用者目錄不會自動啟用。

- 輸入英文：按 `Ctrl+Space` 切到 ABC 模式。
- 常用 emoji 或固定英文片語：加入 Rime `custom_phrase`。

若自行修改 schema 整合第三方字典，該設定與相依檔案需自行維護，不屬於拍台文 0.4.0 發行內容。
