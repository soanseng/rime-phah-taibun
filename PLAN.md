...
| 符號清單 `` ` `` | **台語符號選單**：台羅調號、台文標點、輕聲符（方音符號不收納，0.9.3 後移除） | 替換符號表內容，保留選單 UI 邏輯 | 1 |
...
本節為 v0.6.2 之後的改進路線。設計原則：**維持 Rime 架構**，改進集中在排序演算法、學習行為、資料管線驗證與發佈紀律；不重造引擎、不離開 Rime 生態。

### 9-1. 演算法改進（最優先）

#### A. 整詞優先權重（長詞不敗原則）

**問題**：librime `script_translator` 組句時，逐音節切分的組合權重有機會壓過字典內整詞，讓連續輸入輸出碎片化（例：字典有「橋上」卻拆成兩個單字組句）。

**目標不變量**：字典內 n 音節整詞的權重，必須大於其逐音節拆開候選的組合權重（乘上安全係數），且不傷高頻單字本身的排序。

**做法**：
1. `scripts/build_frequency.py` 分層權重計算後，新增後處理 pass：對每個 n≥2 音節詞 W，檢查 `weight(W) ≥ K × Σ weight(syllable_i)`（K 起始值 1.2，掃描字典校準）；不足者抬升到下限。
2. 校準時抽樣驗證：抬升不應讓低頻長詞湧進候選首頁——以 7 語料庫詞頻為回歸基準。

**驗收**：
- `tests/test_frequency.py` 新增不變量測試：隨機抽樣字典詞 1,000 筆，斷言整詞權重恆 ≥ K×拆分和。
- `tests/test_real_rime.py` 新增案例：字典內長詞（含 4+ 音節詞）在連續輸入下必須以整詞出現，不得拆解。
...
高頻單字（如「的」「我」）在連續輸入時不應搶走整詞/長詞的首位。`phah_taibun_long_word` 已做長詞提升；補整合測試：
- 案例：連續輸入一串「高頻單字音節 + 字典內整詞」，斷言整詞仍以整詞形式優先出現。
- 與 9-1A 的不變量測試互為表裡（權重層 / 行為層各釘一次）。

#### D. 詞身份統一原則：(漢字, 正規化 TL) 成對
...
#### F. Fatal「TL→POJ 轉換完整性」閘

字典同時攜帶 TL 與 POJ 候選，兩者必須由同一轉換器推導一致。在 `scripts/validate_dict.py` 新增全量斷言：**每一個 POJ 候選 == convert(TL 對應讀音)**，失敗即中止 build（不是 warning）。

**動機**：混合來源/多次轉換的管線，最容易出現局部不同步的壞資料；現有 `test_tl_poj_convert.py` 只測轉換器本身，不測產物。

**驗收**：`tests/test_validate.py` 新增案例——植入一筆壞 POJ 的假字典，build 必須失敗。

#### G. Known-keys 最小命中 fixture

build 後對成品字典執行固定查詢集（如 `chi2`、`tsiah8`、`tai5`…），每個 key 設最小命中筆數；低於下限即失敗。下限以當前字典校準，作用是抓「大規模壞版」（轉換 regression、來源漏載）。

**驗收**：fixture 檔進 repo；`tests/` 環境有 librime 時跑真引擎查詢，否則退回文本掃描。

#### H. 字典鍵集 release-diff

每次 build 產出 `(漢字, TL)` 鍵集檔（TSV），隨版封存。release note 可精確寫出「本版新增/移除 N 詞」，也讓字典變更可審、可回溯。

**做法**：`scripts/build_all.py` 尾端產出；檔名帶版號；是否進 repo 或只進 release 附件，實作時決定（預設進 release 附件，避免 repo 膨脹）。

### 9-3. 品質與發佈紀律

#### I. 行為不變量清單

把散在各測試與文件的「打 X 應得 Y」預期集中成一份 status-tracked 清單（`docs/` 或 `tests/` 旁）：每項 = 觸發輸入、預期輸出、狀態（通過/待修）、對應測試。語料例句取自現有 7 語料庫。

**價值**：修 bug 時先加清單項再修；dogfood 有清單可走；新貢獻者一眼看懂系統行為契約。

#### J. 發佈資產雜湊與下載驗證

`release.yml` 增加：資產 SHA256SUMS 檔產出＋上傳；release 後跑一個匿名下載驗證 job（下載、驗雜湊、確認可解）。安裝包未簽章期間，這是使用者驗證來源的唯一管道。

### 9-4. 明確不做（Non-goals）

- **方音符號（TPS）輸入模式**：不開發。POJ/TL 混打已覆蓋羅馬字輸入；方音符號字元亦已自符號選單移除（不收納）。
- **自研輸入法引擎／離開 Rime**：不評估。現有差異化（混打模糊、漢羅、反查）皆以 Rime 架構實現，引擎層重寫無對應收益。
- **原生鍵盤 App／商店上架**：不開發。行動端維持 Trime / fcitx5-android 手動部署路線。
