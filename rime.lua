-- rime.lua
-- 拍台文 Phah Tai-bun Lua 模組註冊
-- 供舊版 librime-lua（不支援 @* 語法）使用

phah_taibun_data     = require("phah_taibun_data")
phah_taibun_input    = require("phah_taibun_input")
phah_taibun_filter   = require("phah_taibun_filter")
phah_taibun_lighttone = require("phah_taibun_lighttone")
phah_taibun_lookup   = require("phah_taibun_lookup")
phah_taibun_help     = require("phah_taibun_help")
phah_taibun_date     = require("phah_taibun_date")
phah_taibun_symbols  = require("phah_taibun_symbols")
phah_taibun_wildcard = require("phah_taibun_wildcard")
phah_taibun_phrase   = require("phah_taibun_phrase")
phah_taibun_synonym  = require("phah_taibun_synonym")
phah_taibun_speedup  = require("phah_taibun_speedup")
phah_taibun_select_char = require("phah_taibun_select_char")
phah_taibun_commit   = require("phah_taibun_commit")
phah_taibun_long_word = require("phah_taibun_long_word")
phah_taibun_recommend = require("phah_taibun_recommend")
phah_taibun_origin   = require("phah_taibun_origin")
phah_taibun_reverse_format = require("phah_taibun_reverse_format")
phah_taibun_telex = require("phah_taibun_telex")
phah_taibun_toneless = require("phah_taibun_toneless")
